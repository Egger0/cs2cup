...
import { checkInForNbt } from '@/lib/nbt'

export async function checkInForNbtAction() {
  ...
  const result = await checkInForNbt(
  ...
  if (result.ok) {
    revalidatePath('/me')
    return { ok: true as const, message: `签到成功，获得 ${result.reward} nbt。` }
  }
  return {
    ok: false as const,
    error:
