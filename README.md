...
Worker Secrets, configure that URL in QQ Open Platform, and subscribe to the group @-message,
`GROUP_MEMBER_ADD`, and `C2C_MESSAGE_CREATE` events. The application validates QQ's signed callback,
accepts group commands only from the configured group OpenID, and welcomes new members. Users bind or
unbind their site username only by private chat with the bot; group binding commands direct users to
private chat. Platform owners can use
