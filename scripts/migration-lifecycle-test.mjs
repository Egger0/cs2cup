...
try {
  ...
  assertSessionFoundationsEmpty(upgradeDatabase)
  assertLifecycle(upgradeDatabase, 'expanded')
  // Exercise the coordinated 014/017 rollback before applying later contract
  // migrations. Append-only histories intentionally refuse holes below 021.
  runMigration(upgradeDatabase, 'contract', { maxVersion: '017' })
  ...
  if (ledgerCount(upgradeDatabase, 'contract') !== 0) {
    throw new Error('rollback did not clear its coordinated contract ledger entries')
  }
