...
try {
  ...
  try {
    ...
    const adminMediaUrl = `${BASE}/admin/media/${uploadedPhotoKey}`
    ...
    const authenticatedPublicDraft = await ctx.request.get(mediaUrl)
    const draftAdminMedia = await ctx.request.get(adminMediaUrl)
    check(
      'Draft media stays hidden on the public route for anonymous and administrator requests',
      initiallyHidden.status() === 404 && authenticatedPublicDraft.status() === 404,
      `anonymous=${initiallyHidden.status()} administrator=${authenticatedPublicDraft.status()}`,
    )
    check(
      'Draft media remains available through the administrator media route',
    ...
    checkPrivateCache('Administrator draft preview is private and non-storable', draftAdminMedia)
    check(
      'Media content type is extension-bound and cannot be sniffed',
      draftAdminMedia.headers()['content-type'] === 'image/png'
        && draftAdminMedia.headers()['x-content-type-options'] === 'nosniff',
      JSON.stringify(draftAdminMedia.headers()),
    )
    check(
      'Media bytes stream without transformation',
      Buffer.compare(await draftAdminMedia.body(), png) === 0,
    )
